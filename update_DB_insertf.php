...
function insertActions($arr, $actionT = 'New Hire'){
    ...
    $SQL= "INSERT INTO tHR_Actions (EEID, ActionType, ReasonCode, StartDate, "
        ...
        .":aT, :aR, '" .date('Y-m-d', strtotime($csv[13])) ."', "
    ...
    $qry->bindParam(':eStat', $csv[9], PDO::PARAM_STR);
    $qry->execute();
...
function insertDate($arr){
    ...
    $SQL = "INSERT INTO tHR_DateSpecs (EmpID, DateType, DateValue) VALUES (:eeID,"
        ."'Original Hire Date', '" .date('Y-m-d', strtotime($csv[13])) . "');";
     //    run the query 
    $dbh = DB_con();
    $qry = $dbh->prepare($SQL);
    $qry->bindParam(':eeID', $csv[1], PDO::PARAM_INT);
...
//     PLXXXXX... is used. To change it to CDXXXXX... format change $csv[7] to $csv[6]
    ...
    $qry->bindParam(':fte', $csv[12], PDO::PARAM_INT);
